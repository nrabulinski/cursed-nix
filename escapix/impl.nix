let
  ...
  getContent =
    ...
    let
      ...
      _valid =
        builtins.match ".*${lib.escapeRegex _startMarker}.*${lib.escapeRegex _endMarker}.*" d != null;
      content = lib.pipe d [
    ...
    in
    if _valid then content else (import drvPath).outPath;
        ...
        let
          ...
          mapStr =
            ...
            let
              ...
              __m = if last then map lib.strings.escapeXML else _recurSubDrv (lib.tail restNames);
              out = __m m;
            in
            lib.concatStringsSep o out;
        in
        map mapStr strs;
      final = _recurSubDrv ctxNames [ str ];
      final' =
        assert (lib.length final == 1);
        lib.head final;
    in
    if builtins.hasContext str then yeet final' else lib.strings.escapeXML str;
